...
public class MidWeatherSchedulerService {
    ...
    public void update() {
        ...
        // 2) 기온 regId 목록
        var regIds = locationRepo.findDistinctMidTermRegCodes();
        if (regIds.isEmpty()) {
    ...
    private void upsert(String regId, LocalDate date, Float pop, SkyCode am, SkyCode pm, Float tmx, Float tmn) {
        var entity = detailRepo.findByMidTermRegCodeAndDate(regId, date)
    ...
    private Float toF(Integer v) { return v == null ? null : v.floatValue(); }
    ...
    private MidLandItem fetchMidLand(String regId, String tmFc) {
        return kmaWebClient.get()
                .uri(uri -> uri
                        ...
                        .queryParam("serviceKey", props.getShortTermKey())
                        .queryParam("dataType", "JSON")
                        .queryParam("tmFc", tmFc)
                        .queryParam("regId", regId)
                        .build())
                .retrieve()
                .bodyToMono(new ParameterizedTypeReference<KmaResponse<MidLandItem>>() {})
                .mapNotNull(resp -> resp.items().stream().findFirst().orElse(null))
                .retryWhen(Retry.backoff(2, Duration.ofMillis(300)).filter(this::isRetryable))
    ...
    private MidTempItem fetchMidTemp(String regId, String tmFc) {
        return kmaWebClient.get()
                .uri(uri -> uri
                        ...
                        .build())
                .retrieve()
                .bodyToMono(new ParameterizedTypeReference<KmaResponse<MidTempItem>>() {})
                .mapNotNull(resp -> resp.items().stream().findFirst().orElse(null))
                .retryWhen(Retry.backoff(2, Duration.ofMillis(300)).filter(this::isRetryable))
