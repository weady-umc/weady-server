...
public interface BoardRepository extends JpaRepository<Board, Long> {
    ...
    @Query("SELECT b "
            ...
            + "AND (:seasonTagIds IS NULL OR b.seasonTag.id IN :seasonTagIds) "
            + "AND (:temperatureTagId IS NULL OR b.temperatureTag.id = :temperatureTagId) "
            + "AND (:weatherTagIds IS NULL OR b.weatherTag.id IN :weatherTagIds) "
            + "AND NOT EXISTS ("
    ...
    Slice<Board> getFilteredAndSortedResults(
            @Param("seasonTagIds") List<Long> seasonTagIds,
            @Param("temperatureTagId") Long temperatureTagId,
            @Param("weatherTagIds")List<Long> weatherTagIds,
